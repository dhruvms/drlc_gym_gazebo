...
class DQNAgent:
    ...
    def create_model_deep(self):  # noqa: D103
    ...
    def create_model_duel(self):  # noqa: D103
        ...
        advantage = Dense(self.num_actions)(fc1)
        fc2 = Dense(512)(flatten)
        value = Dense(1)(fc2)
        policy = merge([advantage, value], mode = lambda x: x[0]-K.mean(x[0])+x[1], output_shape = (self.num_actions,))

        model = Model(input=[input_layer], output=[policy])

        return model

    def compile_linear(self,weights_name):
        # create both networks
        self.q_network = self.create_model_linear()
        # self.target_q_network = self.create_model()

        #load weights from saved file
        self.q_network.load_weights(weights_name)
    ...
    def compile_deep(self,weights_name):
        # create both networks
        self.q_network = self.create_model_deep()
        # self.target_q_network = self.create_model()
    ...
    def compile_duel(self,weights_name):
        # create both networks
        self.q_network = self.create_model_duel()
        # self.target_q_network = self.create_model()
